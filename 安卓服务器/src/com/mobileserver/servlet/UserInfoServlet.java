...
public class UserInfoServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		if (action.equals("query")) {
			...
			/*调用业务逻辑层执行用户信息查询*/
			List<UserInfo> userInfoList = userInfoDAO.QueryUserInfo(user_name,name,birthday,cardNumber,city);
			...
			for (int i = 0; i < userInfoList.size(); i++) {
				...
				.append("</photo>").append("\r\n")
				.append("		<moneyValue>")
				.append(userInfoList.get(i).getMoneyValue())
				.append("</moneyValue>").append("\r\n")
				.append("		<userScore>")
				.append(userInfoList.get(i).getUserScore())
				.append("</userScore>").append("\r\n")
			...
			try {
			  ...
			  stringer.key("moneyValue").value(userInfo.getMoneyValue());
			  stringer.key("userScore").value(userInfo.getUserScore());
				  stringer.endObject();
			  }
			  stringer.endArray();
			} catch(Exception e){}
			...
			userInfo.setMoneyValue(moneyValue);
			int userScore = Integer.parseInt(request.getParameter("userScore"));
