...
public class UserInfoAction extends BaseAction {
    ...
    public String QueryUserInfo() {
        ...
        if(city == null) city = "";
        List<UserInfo> userInfoList = userInfoDAO.QueryUserInfoInfo(user_name, name, birthday, cardNumber, city, currentPage);
        /*计算总的页数和总的记录数*/
        userInfoDAO.CalculateTotalPageAndRecordNumber(user_name, name, birthday, cardNumber, city);
        /*获取到总的页码数目*/
    ...
    public String QueryUserInfoOutputToExcel() { 
        ...
        if(cardNumber == null) cardNumber = "";
        if(city == null) city = "";
        List<UserInfo> userInfoList = userInfoDAO.QueryUserInfoInfo(user_name,name,birthday,cardNumber,city);
        ExportExcelUtil ex = new ExportExcelUtil();
        String title = "UserInfo信息记录"; 
        String[] headers = { "用户名","姓名","性别","生日","身份证号","籍贯","照片","账户余额","会员积分"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<userInfoList.size();i++) {
        	UserInfo userInfo = userInfoList.get(i); 
        	dataset.add(new String[]{userInfo.getUser_name(),userInfo.getName(),userInfo.getSex(),new SimpleDateFormat("yyyy-MM-dd").format(userInfo.getBirthday()),userInfo.getCardNumber(),userInfo.getCity(),userInfo.getPhoto(),userInfo.getMoneyValue() + "",userInfo.getUserScore() + ""});
        }
        /*
