...
public class RoomTypeAddActivity extends Activity {
	// 声明确定添加按钮
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// 设置当前Activity界面布局
		setContentView(R.layout.roomtype_add); 
		ImageView search = (ImageView) this.findViewById(R.id.search);
		search.setVisibility(View.GONE);
		TextView title = (TextView) this.findViewById(R.id.title);
		title.setText("添加房间类型");
			...
			public void onClick(View v) {
				try {
					...
					if(ET_roomTypeName.getText().toString().equals("")) {
						Toast.makeText(RoomTypeAddActivity.this, "房间类型输入不能为空!", Toast.LENGTH_LONG).show();
					...
					/*调用业务逻辑层上传房间类型信息*/
					RoomTypeAddActivity.this.setTitle("正在上传房间类型信息，稍等...");
					String result = roomTypeService.AddRoomType(roomType);
