...
public class NewsInfoEditActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		ET_newsDate = (EditText) findViewById(R.id.ET_newsDate);
			...
			public void onClick(View v) {
				try {
					...
					if(ET_newsContent.getText().toString().equals("")) {
						Toast.makeText(NewsInfoEditActivity.this, "优惠内容输入不能为空!", Toast.LENGTH_LONG).show();
					...
					/*验证获取发布日期*/ 
					if(ET_newsDate.getText().toString().equals("")) {
						Toast.makeText(NewsInfoEditActivity.this, "发布日期输入不能为空!", Toast.LENGTH_LONG).show();
						ET_newsDate.setFocusable(true);
						ET_newsDate.requestFocus();
						return;	
					}
					newsInfo.setNewsDate(ET_newsDate.getText().toString());
					/*调用业务逻辑层上传优惠信息信息*/
	...
	private void initViewData() {
		...
		this.ET_newsDate.setText(newsInfo.getNewsDate());
	}
	@Override
